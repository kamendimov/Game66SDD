...
- `Player.ChangeTrumpCard(int, Card?)` is abstract; `UserPlayer` implements the trump swap and returns the new trump card, while `ComputerPlayer` returns `null`.
...
- `TestTwentyForComputerPlayer()` public method added to `Game66` with an empty body; intended for future computer player twenty-scoring test logic.
...
- `PlayTwenty()` and `PlayForty()` are `virtual` methods in `Player`; they award 20 and 40 points respectively when the player holds the required card combination and return the `Poup` card, `null` otherwise. The caller in `Game66` invokes them only after setting `LastRoundUserWon = true`.
...
- `Game66.DoesTrumpCardPlay()` returns `true` when `PlayCardCount >= sTrumpCardIndex`, indicating the trump card should be displayed.
...
- `PlayTwenty()` and `PlayForty()` methods moved from `Game66` to `Player` as `virtual` methods returning `Card?`; `Game66` sets `LastRoundUserWon` before invoking them in `ProcessUserResult()`.
...
- `PlayTwenty()` is a `virtual` method in `Player` that awards 20 points when the player holds a non-trump value-4 (`Poup`) and value-3 (`Dama`) card of the same suit; implementation iterates over each `Suit` using `Enum.GetValues(typeof(Suit))`, scanning all cards per suit, and returns the `Poup` card immediately when both required cards are found for any non-trump suit.
...
- `PlayScreen_Paint` trump card drawing logic extracted to `private void DrawTrumpCard(Graphics g)`; draws the trump card image at the top right corner when `!mGame66.DoesTrumpCardPlay()` using `TrumpCard.ImagePath`, otherwise draws a suit-based image (`Kare.png`, `Kupa.png`, `Pika.png`, or `Spatia.png`) depending on the trump card's `Suit` enum value (`CardName`).
...
- `TWENTY` (20) and `FORTY` (40) private const int fields added to `Player`; `PlayTwenty()` and `PlayForty()` use them for `IncrementScore`.
